...
**CAS是CPU层面提供的指令**，然后**OS提供api**，在 **Java中通过Unsafe类来进行CAS操作**。（Unsafe底层通过本地方法去调用OS的CAS api）
...
**实现：**Java CAS 操作 依赖 Unsafe类（Unsafe底层通过本地方法去调用OS的CAS api）。
...
- java.util.concurrent.atomic.AtomicInteger#getAndIncrement
  ...
  - 底层调用Unsafe的 **cas 方法**直接**在主内存中进行原子操作**
...
- sun.misc.Unsafe
  ...
  Unsafe类的getAndAddInt()方法**调用cas，若失败则一直自旋**，尝试修改主内存，Unsafe的cas是个本地方法，调用了OS提供的函数进行CPU级别的指令
