...
如下所示，当匿名线程将shutdownRequested改为了true，但是主线程一直dowork阻塞中
...
- 可见性
  ...
  - **volatile**关键字：volatile修饰的变量，可以保证所有线程中该变量的值总是最新的。
...
volatile是 可以保证可见性和有序性，底层依赖的是CPU级别的指令，也就是lock。
...
我们观察加入**volatile关键字**时所生成的**汇编代码**

![image-20220314023422331](images/image-20220314023422331.png)

赋值后（前面mov%eax，0x150(%esi)这句便是赋值操作）多执行了一个  “ **lock  addl  $0x0，(%esp)** ”  操作，通过lock将结果同步到内存时，意味着所有之前的操作都已经执行完成，这个操作的作用相当于一个内存屏障（Memory Barrier，指**重排序时不能把后面的指令重排序到内存屏障之前的位置**），多核同时访问内存时，就需要内存屏障来保证一致性。

“ **addl $0x0，(%esp)** ”，把ESP寄存器的值加0，是一个空操作，**lock前缀**作用是**一旦有线程更新了主内存的某个变量值**，那么**其他线程的工作内存中该变量副本值失效（Invalidate）**



volatile变量读操作的性能消耗与普通变量几乎没有什么差别，但是写操作则可能会慢上一些，因为它需要在本地代码中插入许多内存屏障指令来保证处理器不发生乱序执行。不过即便如此，大多数场景下volatile的总开销仍然要比锁来得更低。我们在volatile与锁中选择的唯一判断依据据仅仅是volatile的语义能否满足使用场景的需求
...
禁止指令重排原理：volatile修饰的变量，经过编译后会在**volailte变量读写操作前后插入 内存屏障**（Memory barrier），**重排序时不能把后面的指令重排序到内存屏障之前的位置**
