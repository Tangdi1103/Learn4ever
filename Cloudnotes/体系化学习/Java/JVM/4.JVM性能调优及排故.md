...
- 使用吞吐量优先的场景：**需要高CPU使用率**的场景，如：**大量运算任务**、**分析任务**、**批处理**、无需太多与客户端交互的任务等
...
JVM 调优不可能一劳永逸，基于现有的硬件资源，随着业务系统不断迭代，可能会出现各种问题，例如经常卡顿、响应变长、甚至出现OOM、CPU过载。我们就需要不断地调整 JVM参数 达到一个量化的性能指标，如吞吐量多少，延迟多少。

总之，JVM调优是围绕内存分配、垃圾回收策略的选择展开的。
...
#### 5. 不断压测、分析、调整直到找到合适的JVM参数配置

#### 6. 找到合适的参数后，将参数应用到该服务的其他实例机器上，后续继续跟踪
