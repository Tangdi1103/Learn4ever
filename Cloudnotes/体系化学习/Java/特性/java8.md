...
其实Lambda就是一个匿名内部类，其实现了函数式接口：Consumer、Supplier、Function<T, R>、Predicate等。

而匿名内部类支持了闭包（匿名内部类中的函数、Lambda在使用函数外部的局部变量，这个局部变量是不能改变的，及隐式的final），示例代码如下：

![image-20220720001103696](images/image-20220720001103696.png)
...
* 并行流：就是把一个内容分成几个数据块，并用不同的线程分别处理每个数据块的流。底层是[ForkJoin框架](../JAVA并发编程/并发工具类/ForkJoin)。
