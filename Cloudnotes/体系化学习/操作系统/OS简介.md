...
<font style=color:red>**操作系统作为系统软件，直接管理与控制计算机硬件（如CPU、内存、文件等），并对用户提供各种接口（如图形界面接口、命令接口、程序接口）**</font>
...
由单道批处理系统可知，程序作业需要运行**<font style=color:red>必须先加载到内存，然后再由CPU[调度](./进程调度与死锁)执行</font>**
...
由操作系统的作业[调度](./进程调度与死锁)程序**按一定策略**从后备作业队列中**选择**若干个**作业调入内存**，使它们**共享CPU和系统中的各种资源**。
...
- <font style=color:red>内存分配</font>
  ...
  **<font style=color:red>前面提到作业被执行，需要先加载进内存，然后被CPU[调度](./进程调度与死锁)执行</font>**。
...
- 进程同步：多个进程的协调与互斥
- 进程通信：进程间的信息交换
- [进程调度](./进程调度与死锁)：多个可执行的进程中选出一个进程，把CPU分配给它
...
![image-20221102120820111](images/image-20221102120820111.png)

当遇到函数调用时，首先会创建一个**栈帧(栈帧是`线程封闭`的)**，存放当前`函数运行所需的数据`以及`函数外的下一行指令地址`，将栈帧压入CPU的**栈寄存器**中，形成`调用栈`。

![image-20221102120832067](images/image-20221102120832067.png)

由于函数调用具有线程封闭的特性，所以在并发编程领域可用来解决并发问题。PS:不同线程执行该函数，都会生成不同的栈帧，栈帧便是函数运行的环境。
