...
同步原理：用<font style=color:red>信号量的值来表示资源的使用情况</font>，以此为基础实现进程同步。信号量类型有整型、记录型、AND型。
...
定义1：管程是描述共享资源的数据结构和在数据结构上的共享资源管理程序的集合。

定义2：管程封装了同步操作，对进程隐蔽了同步细节，简化了同步功能的调用界面。用户编写并发程序如同编写顺序(串行)程序。
