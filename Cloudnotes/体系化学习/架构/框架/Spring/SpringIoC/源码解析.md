...
### 		    3.3.doLoadBeanDefinitions()方法创建XmlBeanDefinitionReader，来加载资源并解析xml的bean，封装成BeanDefinition对象注册到注册器中

![image-20210616145306347](images/image-20210616145306347.png)



### 		    3.4.将BeanDefinition装饰后，id作为key，注册到beanDefinitionMap中，
...
### 	 	    5.1执行BeanDefinitionRegistryPostProcessors处理BeanDefinition注册
...
### 	    8.5.（重复A的过程）从缓存中取bean B，若没有则实例化，若开启循环依赖，则提前暴露，封装成工厂对象放到三级缓存singletonFactories（[查看三级缓存详情](#二、三级缓存解决SpringIoC循环依赖思路图)）

![image-20210616155031795](images/image-20210616155031795.png)

### 	    8.6.bean B继续往下装配属性，发现依赖引用属性bean A，调用getBean(Aname)，发现三级缓存中有bean A，通过工厂对象得到扩展后的A，并放入二级缓存earlysingletonFactories中，bean B装配扩展后的A，然后继续往下初始化，最后bean B放入单例池singletonObjects中
