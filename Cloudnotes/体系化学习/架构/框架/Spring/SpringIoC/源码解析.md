...
### 		    4.3.doLoadBeanDefinitions()方法创建XmlBeanDefinitionReader，来加载资源并解析xml的bean，封装成BeanDefinition对象注册到注册器中
...
### 	 	    6.1执行BeanDefinitionRegistryPostProcessors处理BeanDefinition注册
...
提示：扫描@componentScan配置的bean路径，遍历所有类；通过反射获得带@Configuration注解的类，创建cglib动态代理对象，执行所有带@Bean的方法，增强逻辑实现类的逻辑是：
