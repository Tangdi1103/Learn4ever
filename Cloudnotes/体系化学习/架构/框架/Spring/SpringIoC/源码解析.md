...
但是，最终都会调用**org.springframework.context.support.AbstractApplicationContext#refresh**，去刷新容器
...
## 3. 创建bean工厂、解析bean文件（xml，注解启动不走这步）
...
### 	8.3.从缓存中取bean A，若没有则实例化加载bean A，若开启循环依赖，则提前暴露，放到三级缓存singletonFactories中
...
### 	8.5.（重复A的过程）从缓存中取bean B，若没有则实例化，若开启循环依赖，则提前暴露，放到三级缓存singletonFactories

![image-20210616155031795](images/image-20210616155031795.png)

### 	8.6.bean B继续往下装配属性，发现依赖引用属性bean A，调用getBean(Aname)，发现三级缓存中有bean A，取出并通过beanPostProcesser后置扩展得到代理后的A，并放入二级缓存earlysingletonFactories中，bean B装配扩展后的A，然后继续往下初始化，最后bean B放入单例池singletonObjects中
...
### 	8.7.bean A装配完属性B后，A继续往下进行初始化，最后从二级缓存取出代理后的A覆盖现在的A，最后放入了单例池singletonObjects中
